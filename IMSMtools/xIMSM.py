...
def upsample_input_files(indir, outdir):
    ...
    a high-resolution simulation. As of now (11/10/2019) the
    ...
    outdir: str
        ...
        time step. This function modifies these files in-place.
    """
    f = 'atmosphere.res.nc'
    ...
    # Load datasets
    dsetin = nc.netcdf_file('%s/%s' % (indir, f), mode = 'r')
    dsetout = nc.netcdf_file('%s/%s' % (outdir, f), mode = 'a')
    
    # Generate lat-lon grids
    latin = gglat(dsetin.variables[lat].data.size)
    lonin = np.linspace(0.0, 360.0, dsetin.variables[lon].data.size + 1)
    latout = gglat(dsetout.variables[lat].data.size)
    lonout = np.arange(0.0, 360.0, 360.0/dsetout.variables[lon].data.size)
    
    for v in dsetin.variables.keys():
        ...
        arrin = dsetin.variables[v].data
        arrout = dsetout.variables[v].data
        ...
        dsetout.variables[v].data = arrout
